...
struct MatchRGBD{
    ...
    Vector3f    obs3d;
    Vector3f    mdl3d;
    pair<int, int> mdlidx;  // model index and model feature index
    int         obsidx;     // detected feature index
...
/** match descriptors
  ...
  * @return matches
  */
void display_matches( const cv::Mat & image, const vector< vector<MatchRGBD> > & matches, const vector< vector<int> > & clusters, const vector< vector<int> > & clustered_matches );
