...
/** constructor
  ...
  */
DataWriter::DataWriter(ros::NodeHandle &nh, std::string dir, bool use_cloud) {
    nh_ = &nh;
    dir_= dir;
    count_ = 0;
...
void DataWriter::sensor_callback(const sensor_msgs::ImageConstPtr &xtion_rgb_msg,
    ...
    if ( key == 's' ) {
        ...
        MultiMouseCallback xtion_depth_cb( vis_xtion_depth );
        ...
        std::string xtion_rgb_mask_name = dir_ + "/mask_xtion_rgb_" + boost::lexical_cast<std::string>(count_) + ".png";
        std::string xtion_depth_mask_name = dir_ + "/mask_xtion_depth_" + boost::lexical_cast<std::string>(count_) + ".png";
        std::string camera_rgb_mask_name = dir_ + "/mask_camera_rgb_" + boost::lexical_cast<std::string>(count_) + ".png";
        ...
        */
        pcl::console::print_value( "Save No. %d frame to disk\n", count_ );
        count_ ++;
    }
}
