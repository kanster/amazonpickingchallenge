/*
 * helpfun.h
 * Description: help functions in Kernel Descriptors
 * Includes:
 *      - eval_kernels
 *      - scaletest
 *      - scaletrain
 ...
 *
 * Author: Xiaofeng Ren, modifiied by Kanzhi Wu
 * Date: 20/03/2015
 *
 */
...
///** @todo delete later, direct from setting up */
...
inline int sgn(float val) { return (0.0 < val) - (val < 0.0); }
...
static MatrixXf scaletest_power( const MatrixXf& imfea, const MatrixXf& minvalue, const MatrixXf& maxvalue) {
    MatrixXf imfea_s(imfea.rows(), imfea.cols());
    for ( int y = 0; y < imfea_s.rows(); ++ y ) {
        for ( int x = 0; x < imfea_s.cols(); ++ x ) {
            imfea_s(y, x) = sgn(imfea(y,x))*pow( abs(imfea(y,x)), 0.3 );
        }
    }
    return imfea_s;
}
...
//    #pragma omp parallel for
    ...
    // cout << "EvalKernelExp_Img3 3\n";


    (result) *= params(0,0);
    (result).array() = result.array().exp();
    // cout << "EvalKernelExp_Img3 4\n";
...
//    cout << kparam << endl;
    ...
    float rem_x = ( (img_w-patch_size) % grid_space );//( (img_w-patch_size-1) % grid_space )+1;
    float rem_y = ( (img_h-patch_size) % grid_space );

    float offset_x = floor(rem_x/2) + 1;
    float offset_y = floor(rem_y/2) + 1;
//    printf( "grid : %d, rem_x : %f, rem_y: %f, offset_x: %f, offset_y: %f\n", grid_space, rem_x, rem_y, offset_x, offset_y );
    // meshgrid
    int multiplier_x = floor((img_w-patch_size+1 - offset_x)/grid_space) + 1;
    int multiplier_y = floor((img_h - patch_size + 1 - offset_y)/grid_space) + 1;
    RowVectorXf grid_x_v = RowVectorXf(multiplier_x);
