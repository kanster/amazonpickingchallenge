...
vector<MatchRGBD> FeatureMatcher::process(vector<DetectedFeatureRGBD> &features, int object_in_bin, vector<list<int> > &match_clusters ) {
    ...
    int n_match = 0;
    for ( size_t i = 0; i < features.size(); ++ i ) {
        normaliser( features[i].descrip );
        for ( size_t j = 0; j < 128; ++ j )
            pt[j] = features[i].descrip[j];
        #pragma omp critical(ANN)
        kdtree_->annkSearch( pt, 2, nx, ds, 5 );
        if ( ds[0]/ds[1] < 0.8 ) {
            if ( matches.capacity() < 1000 )
                matches.reserve( 1000 );
            if ( corresp_model_idx_[nx[0]] == object_in_bin ) {
                matches.resize( matches.size() + 1 );
                MatchRGBD & match = matches.back();
                match.img2d = features[i].img2d;
                match.obs3d = features[i].obs3d;
                match.mdl3d = *corresp_model_feat_[nx[0]];
                match.mdlidx = make_pair<int, int>( corresp_model_idx_[nx[0]], corresp_feat_idx_[nx[0]] );
                match.cluidx = features[i].groupidx;
                match.obsidx = i;
                list<int> & cluster = clusters[match.cluidx];
                cluster.push_back( n_match );
                n_match ++;
            }
        }
    }
    ...
    for ( map<int, list<int> >::iterator it = clusters.begin(); it != clusters.end(); ++ it ) {
        match_clusters.push_back( it->second );
    }
    clusters.clear();
    delete kdtree_;
    corresp_model_idx_.clear();
    corresp_model_feat_.clear();
    corresp_feat_idx_.clear();
